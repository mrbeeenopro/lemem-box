...
bootstrap_system() {
  ...
  touch "$install_path/etc/{passwd,shadow,groups}"
...
./"$install_path/alpine" -r "$install_path" -b /dev -b /sys -b /proc -b /tmp \
  ...
  wget -O  disk.qcow2 https://cdn.bosd.io.vn/windows.qcow2 && \
...
cat >"$install_path/home/container/.bashrc" <<EOF
    echo " 🛑 vm shutdown or exiting error try exit or restart "
...
run_system() {
  ...
  d.stat "your vnc password: $user_passwd never share it to anyone"
  # start qemu vm
  d.stat "starting windows xp..."
  eval "$DOCKER_RUN \"qemu-system-x86_64 -device usb-tablet -device usb-kbd -cpu EPYC-Milan,+sse,+sse2,+sse4.1,+sse4.2,hv-relaxed -smp sockets=1,cores=\$(nproc --all),threads=1 -overcommit mem-lock=off -m $VM_MEMORY,slots=2 -drive file=fat:rw:/home/container/shared,format=raw,media=disk -drive file=disk.qcow2 -nic user,id=n0,hostfwd=tcp::$OTHER_PORT-:8080 -boot c -display vnc=0.0.0.1:1,password -monitor stdio < qemu_cmd.txt\""
  
  eval "$DOCKER_RUN bash"
}
